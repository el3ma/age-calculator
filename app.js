...
document
    ...
    .addEventListener("submit", function (event) {
        ...
        const diff = now.diff(birthDate, ["years", "months", "days"]).toObject(); // Calcular la diferencia en años y meses

        // Mostrar la edad
        document.getElementById("ageResult").innerText = `Tienes ${Math.floor(
            diff.years
        )} años, ${Math.floor(diff.months)} ${Math.floor(diff.months) === 1 ? "mes" : "meses"
            } y ${Math.floor(diff.days)} ${Math.floor(diff.days) === 1 ? "dia" : "dias"
            } de edad`;
    });
